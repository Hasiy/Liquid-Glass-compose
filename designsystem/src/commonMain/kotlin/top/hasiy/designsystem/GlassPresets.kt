 ...
 *
 * CMP 化後配色不再讀 Android 資源（原 `res/values/colors.xml` 已刪除）：
 * 權威定義是本檔案底部的 `THEME_*` 常量，靜態欄位仍作為非 Composable 情境的
 * 回退值。要換色的使用端請 `copy()` 出自己的 [GlassConfig]，資源覆蓋機制不再提供。
 */
object GlassPresets {
    ...
    /**
     * 水滴主題。結構參數（透明度、圓角、陰影）沿用 [Drop]，顏色取 [THEME_COLORS] 的水滴配色。
     *
     * 保留 @Composable 簽名以維持來源相容；取色已不依賴 Composition。
     */
    @Composable
    fun drop(): GlassConfig = Drop.withColorsFrom(THEME_COLORS.drop)

    /** 中性主題，取色方式同 [drop] */
    @Composable
    fun neutral(): GlassConfig = Neutral.withColorsFrom(THEME_COLORS.neutral)

    /** 深色玻璃主題，取色方式同 [drop] */
    @Composable
    fun dark(): GlassConfig = Dark.withColorsFrom(THEME_COLORS.dark)

    /** 原生主題，取色方式同 [drop] */
    @Composable
    fun native(): GlassConfig = Native.withColorsFrom(THEME_COLORS.native)

    /** 所有主題，順序與 [All] 一致，供設定頁展示 */
    @Composable
...
/**
 * 四組結構主題的配色，原 `res/values/colors.xml` 的權威色值在此落戶。
 *
 * 注意與靜態欄位的差異：靜態欄位是「非 Composable 情境的回退值」，個別顏色
 * （如 Neutral 的 base、各主題的頁面漸層底色）只有這裡才是完整定義。
 */
private object THEME_COLORS {
    /** 深色頁面漸層（水滴 / 深色 / 原生共用） */
    private val PAGE_TOP_DARK = Color(0xFF2A1630)
    private val PAGE_BOTTOM_DARK = Color(0xFF0B0D1A)

    /** 強調色預設值（各主題共用；僅在 accentEnabled 為 true 時生效） */
    private val ACCENT = Color(0xFF00A15C)

    val drop = ThemeColors(
        base = Color(0xFF9A9AA8),
        content = Color.White,
        highlight = Color.White,
        border = Color.White,
        pageTop = PAGE_TOP_DARK,
        pageBottom = PAGE_BOTTOM_DARK,
        accent = ACCENT,
    )

    val neutral = ThemeColors(
        base = Color(0xFFF7F7F7),
        content = Color(0xFF111111),
        // 淺色表面的柔光是「壓暗」，所以高光色是黑的
        highlight = Color.Black,
        border = Color.Black,
        pageTop = Color(0xFFE3E3E3),
        pageBottom = Color(0xFFD1D1D1),
        accent = ACCENT,
    )

    val dark = ThemeColors(
        base = Color(0xFF2E2E3A),
        content = Color.White,
        highlight = Color.White,
        border = Color.White,
        pageTop = PAGE_TOP_DARK,
    ...
    val native = ThemeColors(
        base = Color(0xFFCFCFD8),
        content = Color.White,
        highlight = Color.White,
        border = Color.White,
        pageTop = PAGE_TOP_DARK,
...
/** 把一組配色套到既有主題的結構參數上 */
private fun GlassConfig.withColorsFrom(colors: ThemeColors): GlassConfig = copy(
    baseColor = colors.base,
    contentColor = colors.content,
    highlightColor = colors.highlight,
    borderColor = colors.border,
    pageBackgroundTop = colors.pageTop,
    pageBackgroundBottom = colors.pageBottom,
