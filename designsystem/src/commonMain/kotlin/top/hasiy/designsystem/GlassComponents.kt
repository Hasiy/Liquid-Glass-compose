...
fun GlassThemeSelector(
    ...
    Row(
        ...
        // 用 themed() 而非 All：配色取 GlassPresets 的权威定义，与靜態欄位的回退值可能不同
 ...
 *
 * 用索引而不是拿 [GlassConfig] 去比對：themed() 帶完整配色，與靜態欄位
 * （[GlassPresets.Drop] 等回退值）本就不相等，使用端 `copy()` 後更會失準。
 *
 * 去資源化後文案為 SDK 內建預設（原 strings.xml），本地化由使用端自繪入口文字承擔。
 */
private val PRESET_NAMES = listOf("水滴", "中性", "深色", "原生")
