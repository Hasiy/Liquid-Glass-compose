...
class SurfaceTokenFallbackTest {
    ...
    fun `track surface uses the palette token`() {
        ThemePalette.All.forEach { palette ->
            ...
            assertEquals(palette.track.toInt(), config.asTrackSurface().baseColor.toArgb(), "${palette.id.id} track")
        }
    }
    ...
    fun `quiet accent uses the palette token when given`() {
        ThemePalette.All.filter { it.quietAccent != 0L }.forEach { palette ->
            val config = palette.toGlassConfig()
            assertEquals(palette.quietAccent.toInt(), config.quietAccentColor.toArgb(), "${palette.id.id} quietAccent")
    ...
    fun `fill surface builds an accentLight to accent gradient`() {
        ThemePalette.All.forEach { palette ->
            ...
            assertEquals(palette.accentLight.toInt(), fill.baseColor.toArgb(), "${palette.id.id} 漸層起點")
            assertEquals(palette.accent.toInt(), fill.bodyEndColor.toArgb(), "${palette.id.id} 漸層終點")
        }
    }
    ...
    fun `other surfaces leave the gradient end unspecified`() {
        ...
        listOf(
            ...
            assertEquals(Color.Unspecified, surface.bodyEndColor, "$name 不該有漸層終點")
        }
    }
    ...
    fun `overlay surface uses glass and glassDeep`() {
        ThemePalette.All.forEach { palette ->
            val config = palette.toGlassConfig()
            assertEquals(palette.glass.toInt(), config.asOverlaySurface().baseColor.toArgb(), "${palette.id.id} glass")
            assertEquals(palette.glassDeep.toInt(), config.asOverlaySurface(deep = true).baseColor.toArgb(), "${palette.id.id} glassDeep")
        }
    }
