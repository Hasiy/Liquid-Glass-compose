...
class SelectedSurfaceContentColorTest {
    ...
    fun `strong selection uses onAccent as its foreground`() {
        ThemePalette.All.forEach { palette ->
            ...
            assertEquals(palette.onAccent.toInt(), config.asSelectedSurface(strong = true).contentColor.toArgb(), "${palette.id.id} strong 前景")
        }
    }
    ...
    fun `light selection uses accentDeep as its foreground`() {
        ThemePalette.All.forEach { palette ->
            val config = palette.toGlassConfig()
            assertEquals(palette.accentDeep.toInt(), config.asSelectedSurface(strong = false).contentColor.toArgb(), "${palette.id.id} 淡底前景")
        }
    }
