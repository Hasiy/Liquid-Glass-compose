...
class SevenSegmentMapTest {
    ...
    fun `each digit lights the expected number of segments`() {
        ...
        expected.forEach { (digit, count) ->
            assertEquals(count, sevenSegmentsFor(digit)!!.length, "$digit 的段数")
        }
    }
    ...
    fun `every entry only references segments a through g`() {
        ('0'..'9').forEach { digit ->
            ...
            assertTrue(segments.all { it in 'a'..'g' }, "$digit 含非法段：$segments")
            assertEquals(segments.length, segments.toSet().size, "$digit 有重复段：$segments")
        }
    }
    ...
    fun `punctuation and letters are not digits`() {
        listOf('.', ':', ',', ' ', 'k', '级').forEach {
            assertNull(sevenSegmentsFor(it), "$it 不该有段")
        }
    }
}
