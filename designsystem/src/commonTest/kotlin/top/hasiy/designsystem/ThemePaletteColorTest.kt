...
class ThemePaletteColorTest {
    ...
    fun `palette colors keep srgb color space`() {
        ThemePalette.All.forEach { palette ->
            ...
            named.forEach { (token, color) ->
                assertEquals(ColorSpaces.Srgb, color.colorSpace, "${palette.id.id}.$token color space")
            }
        }
    }
    ...
    fun `palette colors round trip to the original argb`() {
        ThemePalette.All.forEach { palette ->
            val glass = palette.toGlassPalette()
            assertEquals(palette.danger.toInt(), glass.danger.toArgb(), "${palette.id.id}.danger")
            assertEquals(palette.screen.toInt(), glass.screen.toArgb(), "${palette.id.id}.screen")
            // 帶 alpha 的令牌也要原樣還原：浮層濃度錯了整個玻璃感就沒了
            assertEquals(palette.glass.toInt(), glass.glass.toArgb(), "${palette.id.id}.glass")
        }
    }
    ...
    fun `material color scheme builds for every palette`() {
        ThemePalette.All.forEach { palette ->
            ...
            assertEquals(palette.danger.toInt(), scheme.error.toArgb(), "${palette.id.id}.error")
            assertEquals(palette.accent.toInt(), scheme.primary.toArgb(), "${palette.id.id}.primary")
        }
    }
    ...
    fun `glass config builds for every palette`() {
        ThemePalette.All.forEach { palette ->
            ...
            assertEquals(palette.accent.toInt(), config.accentColor.toArgb(), "${palette.id.id}.accentColor")
            // 語意色讀取入口也要能安全求值——多數會走 copy()/lerp()，都要讀 color space
            assertEquals(palette.danger.toInt(), config.dangerColor.toArgb(), "${palette.id.id}.dangerColor")
            assertEquals(palette.background.toInt(), config.canvasColor.toArgb(), "${palette.id.id}.canvasColor")
