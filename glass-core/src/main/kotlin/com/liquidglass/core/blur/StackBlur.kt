...
/**
 * StackBlur：O(n) 近似高斯模糊，作用于降采样位图（SPEC-02 §2.2 步骤 3）。
 * 替代已废弃的 RenderScript，纯 Kotlin 实现，在 Dispatchers.Default 上执行。
 */
object StackBlur {
    ...
    fun blur(bitmap: Bitmap, radius: Int) {
        ...
        divsum = divsum shr 1
        divsum *= divsum
        for (i in vmap.indices) vmap[i] = i / divsum
