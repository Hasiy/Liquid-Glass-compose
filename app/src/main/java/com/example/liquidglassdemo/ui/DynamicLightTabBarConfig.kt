...
object DynamicLightTabBarConfig {
     ...
     * 0.8 → 光暈約一個 item 大小；>1 會擴散到相鄰 item。
     */
    const val BAR_GLOW_RADIUS_FACTOR = 0.8f

    /** 整條 Bar 的光暈強度（0.0~1.0）；越高越亮 */
    const val BAR_GLOW_INTENSITY = 0.3f

    /**
     * Pill 內補光半徑倍率（相對於 item 寬度）。
     * 光暈只在接近手指時微微提亮選中項。
     */
    const val INDICATOR_GLOW_RADIUS_FACTOR = 0.4f

    /** Pill 內補光強度（0.0~1.0） */
    const val INDICATOR_GLOW_INTENSITY = 0.12f
    ...
    // ---------- 動畫 ----------
    /** 選中縮放反彈目標最小值（0.9 = 縮到 90%） */
    const val SELECTED_SCALE_MIN = 0.90f

    /** 縮放反彈回彈目標（1.0） */
    const val SELECTED_SCALE_NORMAL = 1f

    /** 縮放反彈延遲（ms） */
    const val SELECTED_SCALE_DELAY_MS = 70L
